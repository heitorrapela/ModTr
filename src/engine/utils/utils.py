...
import torchvision
from .fusion_strategy import addition_fusion, attention_fusion_weight
...
class Utils():
    ...
    def show_bbox(img, bboxes, color=None, label=None, score=None, position=None, decode_dict={'0' : 'Person'}, format='xyxy'): 
        ...
        for index, bbox in enumerate(bboxes):
            ...
            if label is not None: #and bbox[-1] >= 0:
                tlabel = label #str(int(bbox[-1])) if self.labels is None else label #decode_dict[str(int(bbox[-1]))]
                if isinstance(label, list):
                    actual_key = int(label[index])
                    actual_key = actual_key if actual_key in decode_dict else 0
                    tlabel = str(decode_dict[actual_key])
    ...
    def plot_each_image(image, output, target, threshold=0.5):
        ...
        labels_th[(labels_th < 0) | (labels_th >= len(decode_dict))] = 0
        ...
        if((len(boxes_th) > 0)):
            ...
            image, boxes_th.type(torch.int), color=(255,0,0), 
                label=labels_th.cpu().numpy().tolist(),
                position="top", 
    ...
    @staticmethod
    def plot_each_image_norm(image):
    ...
    def get_bbox(filename, dataset='kaist', train=False):
        ...
        for obj in root.findall("object"):
            ...
            # Just to make sure that the min is min and max is max (filter bbox between img size avoid problems when conver bbox to float)
            xmin = max(0, min(bbox[0], bbox[2]))
            ymin = max(0, min(bbox[1], bbox[3]))
            xmax = min(max(bbox[0], bbox[2]), width-1)
            ymax = min(max(bbox[1], bbox[3]), height-1)
            ...
            if(dataset == 'flir'):
                ...
                elif(not train): # Filter like the above link, get bbox greater than 50 height for test
                    if(obj.find("name").text == "person" and abs(ymax-ymin) > 50.0):
                        bboxes.append(bbox)
                        labels.append([1])

                    if(obj.find("name").text == "bicycle" and abs(ymax-ymin) > 10.0):
                        bboxes.append(bbox)
                        labels.append([2])

                    if(obj.find("name").text == "car" and abs(ymax-ymin) > 10.0):
                        bboxes.append(bbox)
                        labels.append([3])
    ...
    @staticmethod
    def fusion_data(imgs_translated, img_ir, fuse_data="none"):

        if(fuse_data == "cross"):
            imgs_translated = torch.clip(img_ir * imgs_translated, min=0.0, max=1.0)
        elif(fuse_data == "addition"):
            imgs_translated = addition_fusion(img_ir, imgs_translated)
        elif(fuse_data == "attention"):
            imgs_translated = attention_fusion_weight(img_ir, imgs_translated)

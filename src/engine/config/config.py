...
class Config:
    ...
    class Dataset:
        ...
        if dataset == 'kaist': 
            train_path = os.path.join(dataset_root, 'kaist/train')
            valid_path = None # validation is created on the fly (with same seed you get same imgs)
            test_path = os.path.join(dataset_root, 'kaist/test')
            ext = '.png'
            n_classes = 2 # person and background
        elif dataset == 'llvip':
            train_path = os.path.join(dataset_root, 'LLVIP/')
            valid_path = None
            test_path = os.path.join(dataset_root, 'LLVIP/')
            ext = '.jpg'
            n_classes = 2
        elif dataset == 'flir':   
            train_path = os.path.join(dataset_root, 'FLIR_aligned/')
            valid_path = None
            test_path = os.path.join(dataset_root, 'FLIR_aligned/')
            ext = '.jpg'
            n_classes = 4
        else:
            train_path = None
    ...
    class EncoderDecoder:
        ...
        encoder_decoder_load_path = 'lightning_logs/modtr/detector_fasterrcnn_modtr_det01reg01_flir_200ep_coco_seed123_fusenone_accgradb2_schcossan/flir_ir_fasterrcnn/best.ckpt'
    ...
    @staticmethod
    def argument_parser():
        parser = argparse.ArgumentParser(description='ModTr')

        parser.add_argument('--dataset', type=str, default='llvip', help='kaist/llvip/flir')
        ...
        parser.add_argument('--detector', type=str, default='fasterrcnn', help="Model Path (default: fasterrcnn)")
        ...
        parser.add_argument('--modality', type=str, default='ir', help='Modality: rgb or ir (default: ir)')
        ...
        parser.add_argument('--wandb-project', type=str, default="modtr", help='Wandb Project Name')
        ...
        parser.add_argument("--batch", type=int, default=1, help="Batch size (default: 16)")
        ...
        parser.add_argument("--decoder-encoder-depth", type=int, default=5, help='5')
        
        parser.add_argument("--decoder-encoder-weights", type=str, default='imagenet', help='imagenet')
        
        parser.add_argument("--decoder-in-channels", type=int, default=3, help=3)
        
        parser.add_argument("--decoder-out-channels", type=int, default=3, help=3)
        ...
        parser.add_argument("--pre-train-path", type=str, default=None, help="Modtr Path")
